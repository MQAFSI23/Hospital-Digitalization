    ...
    <div class="py-10">
        ...
        <form action="{{ route('admin.daftarPengguna') }}" method="GET" class="mt-8">
            ...
            <div class="flex flex-col sm:flex-row sm:space-x-4 mb-4">
                ...
                <button type="submit" class="bg-indigo-500 hover:bg-indigo-700 text-white py-2 px-4 rounded">
                    Filter
                </button>
            </div>
        </form>
        ...
        <div class="overflow-hidden bg-white p-6 rounded shadow-lg mt-2">
            <div class="overflow-x-auto max-h-96 mt-2">
                <table class="min-w-full border-collapse table-fixed">
                    <thead class="bg-indigo-500 text-white sticky top-0 z-10">
                        <tr>
                        ...
                        @foreach ($daftarPengguna as $index => $user)
                            <tr class="{{ $index % 2 === 0 ? 'bg-gray-100' : 'bg-white' }}">
                                ...
                                <td class="py-2 px-4">
                                    ...
                                    <form action="{{ route('admin.hapusPengguna', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus pengguna ini?');">
                                        @csrf
                                        @method('DELETE')
